import { getConfirmedRecords, createConfirmedRecord, updateConfirmedRecord, deleteConfirmedRecord, getCampaignConfirmedRecords, logout } from '../api.js';
...
export class ConfirmedTrackerPage {
  ...
  bindEvents(container) {
    ...
    container.querySelector('#addRecordBtn').addEventListener('click', () => {
      this.editingId = null;
      container.querySelector('#modalTitle').textContent = 'Add Confirmed Record';
    ...
    container.querySelector('#recordForm').addEventListener('submit', async (e) => {
      ...
      const data = {
        ...
        confirmed_date: container.querySelector('#confirmed_date').value,
      ...
      try {
        if (this.editingId) {
          await updateConfirmedRecord(this.editingId, data);
        } else {
          await createConfirmedRecord(data);
        }
        container.querySelector('#modal').classList.add('hidden');
  ...
  clearForm(container) {
    ...
    container.querySelector('#confirmed_date').value = '';
  ...
  async loadRecords(container) {
    try {
      ...
      if (user.role !== 'admin' && user.campaign) {
        data = await getCampaignConfirmedRecords(user.campaign);
      } else {
        data = await getConfirmedRecords();
      }
      ...
      console.error('Failed to load confirmed records:', error);
    }
  }
  ...
  renderCampaignStats(container, records) {
    ...
    statsContainer.innerHTML = campaigns.map(campaign => {
      ...
      const colors = ['emerald', 'green', 'teal', 'cyan'];
  ...
  renderTable(container, records, filterCampaign = null) {
    ...
    if (showGrouped) {
      campaignKeys.forEach((campaign, campaignIndex) => {
        ...
        const todayCount = campaignRecords.filter(r => r.confirmed_date === today).length;
              ...
              <tbody class="divide-y divide-gray-100">
                ${campaignRecords.map((record, index) => `
                  <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-6 py-4 text-sm text-gray-800 font-medium">${record.contact_date || '-'}</td>
                    <td class="px-6 py-4 text-sm text-gray-800">${record.agent_name}</td>
                    <td class="px-6 py-4 text-sm text-gray-800 font-mono">${record.loan_number}</td>
                    <td class="px-6 py-4 text-sm text-gray-800 font-medium">${record.confirmed_date || '-'}</td>
                    <td class="px-6 py-4 text-sm text-green-700 font-semibold">₱${parseFloat(record.ptp_amount).toLocaleString('en-PH', { minimumFractionDigits: 2 })}</td>
                    <td class="px-6 py-4 text-sm text-gray-500 max-w-[120px] truncate">${record.remarks || '-'}</td>
                    <td class="px-6 py-4 text-sm">
                      <div class="flex gap-2">
                        <button class="edit-btn px-2.5 py-1.5 bg-emerald-50 text-emerald-700 rounded-lg hover:bg-emerald-100 transition-colors font-medium text-xs" data-id="${record.id}">Edit</button>
                        <button class="delete-btn px-2.5 py-1.5 bg-red-50 text-red-700 rounded-lg hover:bg-red-100 transition-colors font-medium text-xs" data-id="${record.id}">Delete</button>
                      </div>
                    </td>
                  </tr>
                `).join('')}
      ...
      if (displayRecords.length === 0 && filterCampaign) {
        ...
        tbody.innerHTML = displayRecords.map((record, index) => `
          <tr class="hover:bg-emerald-50/50 transition-colors border-b border-gray-100 last:border-0">
            ...
            <td class="px-4 py-3">
              <span class="text-sm text-gray-800 font-medium bg-emerald-50 px-2 py-1 rounded-md">${record.confirmed_date || '-'}</span>
            </td>
            ...
            <td class="px-4 py-3">
              <div class="flex gap-1.5 justify-center">
                <button class="edit-btn px-2.5 py-1.5 bg-emerald-50 text-emerald-600 rounded-lg hover:bg-emerald-100 hover:text-emerald-700 transition-all font-medium text-xs flex items-center gap-1" data-id="${record.id}">
  ...
  updateStats(container, records) {
    ...
    const today = new Date().toISOString().split('T')[0];
    const todayCount = records.filter(r => r.confirmed_date === today).length;
  ...
  editRecord(container, id) {
    ...
    this.editingId = id;
    container.querySelector('#modalTitle').textContent = 'Edit Confirmed Record';
    ...
    container.querySelector('#confirmed_date').value = record.confirmed_date || '';
  ...
  confirmDelete(container, id) {
    ...
    newConfirmBtn.addEventListener('click', async () => {
      try {
        await deleteConfirmedRecord(id);
  ...
  filterRecords(container, query) {
    ...
    const filtered = this.records.filter(r =>
      ...
      (r.confirmed_date && r.confirmed_date.includes(query))
    );
